...
要求：
1. 图集页：沉浸式；
2. 页面布局：1-Tabs标签(距离瀑布流10；每个Item内容完全显示，显示不全可滑动)+瀑布流
2. 标签使用Tabs实现，标签位于刘海/灵动岛下方
3. 切换到对应标签后，显示响应标签的图片信息
4. 标签下，图片以瀑布流的形式展示，Item宽高比(1:2)
5. 图片接口使用api/images，接口返回值与VideoData类如ImageData
6. 瀑布流Item:1-以图片为背景(平铺)；2-浮动阴影区(1-图片名；2-点赞(左,赋值1-1000随机数)+评论(右，,赋值1-1000随机数) 3-阴影区有背景色；)
7. 点击每个Item：
   -全屏查看：1-点击图片弹出全屏查看层(瀑布流在底部可看到)，再次点击退出，回到瀑布流默认状态；2-支持双指缩放(捏合手势，范围1-4倍)
   -退出方式：轻触图片或背景区域
8.遵循MVVM开发模式，自定义View放到Album/view下   
```
